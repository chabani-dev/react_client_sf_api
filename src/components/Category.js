...
export default function Single() {
    ...
    const url=`http://localhost:8000/api/posts_by_category/${name}`
                ...
                <div className="m-5">
                    ...
                    <a href={`/post/${post.id}`} key={index} className="text-decoration-none">
                        <div className="border mb-2 p-2">
                            ...
                            <a className="text-decoration-none" href={`/auteur/${post.author.login}`}><p className="text-secondary">Écrit par <em className="fw-bold text-body">{post.author.login}</em></p></a>
                        </div>
                    </a>
                    )})}
                </div>
                <a className="btn btn-light ms-5" href="/">Retour à l'accueil</a>
            </>
        );
    }
}